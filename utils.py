...
def compute_ecg_bandpowers(
    ...
    """
    Apply QC filter to ECG and compute PSD and bandpowers.
    ...
    hp : float
        High-pass frequency for QC filter
    notch_hz : float
        Notch frequency for powerline
    notch_q : float
        Notch Q factor
    nperseg_sec : float
    ...
    -------
    dict
        'P_in', 'P_out', 'ratio', 'f', 'Pxx' (filtered signal PSD)
    """
    # 1) QC filter
    ecg_qc = ecg_qc_filter(ecg_signal, fs=fs, hp=hp, notch_hz=notch_hz, notch_q=notch_q)

    # 2) Welch PSD
    nperseg = int(round(nperseg_sec * fs))
    nperseg = min(nperseg, len(ecg_qc))
